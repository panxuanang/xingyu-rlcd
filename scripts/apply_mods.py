...
# Xingyu V7
# - Keep the V6 dynamic large chat panel and shrunken avatar.
# - Long replies scroll once from top to bottom and stop.
# - Slow the movement to 70 ms/pixel (about 14.3 px/s).
# - Wait 1.5 seconds before scrolling, closer to the original firmware feel.
# - No maximum duration cap, so very long replies do not speed up artificially.
...
# ---------------------------------------------------------------------------
# 3) Custom display: dynamic big chat + slower one-way scrolling.
# ---------------------------------------------------------------------------
...
void CustomLcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (chat_status_label_) {
        ...
        if (label_h > visible_h) {
            // V7: deliberately slower than both V6 and the original firmware.
            // V6 was 22 ms/pixel with a 300 ms delay.
...
print("Xingyu V7: expanded chat + slow 70ms/pixel one-way RLCD scrolling applied successfully")
